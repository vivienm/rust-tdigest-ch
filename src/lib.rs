...
impl TDigest {
    ...
    {
        Quantiles {
            inner: Some(self.quantiles_inner(levels)),
        }
    }

    fn quantiles_inner<I>(&mut self, levels: I) -> QuantilesInner<I>
    where
        I: IntoIterator<Item = f64>,
    {
        ...
        if self.centroids.is_empty() {
            return QuantilesInner::Repeat(levels, f32::NAN);
        }

        self.compress();
        let centroid = *self.centroids.first().unwrap();

        if self.centroids.len() == 1 {
            return QuantilesInner::Repeat(levels, centroid.mean);
        }

        let level = match levels.next() {
            None => {
                return QuantilesInner::Exhausted;
            }
            Some(level) => level,
        };
        QuantilesInner::NextCentroid(
            levels,
            QuantilesVariables {
                centroids: self.centroids.iter(),
                count: self.count,
                x: level * self.count as f64,
                prev_x: 0.0,
                current_x: 0.0,
                c: centroid,
                prev_c: centroid,
...
/// An iterator that yields estimated quantiles for a t-digest.
///
/// This `struct` is created by the [`quantiles`] method on [`TDigest`].
/// See its documentation for more.
///
/// [`quantiles`]: TDigest::quantiles
pub struct Quantiles<'a, I>
where
    I: IntoIterator<Item = f64>,
{
    inner: Option<QuantilesInner<'a, I>>,
}

impl<'a, I> Iterator for Quantiles<'a, I>
where
    I: IntoIterator<Item = f64>,
{
    type Item = f32;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let (inner, quantile) = inner.into_next();
        self.inner = Some(inner);
        quantile
    }
}

impl<'a, I> FusedIterator for Quantiles<'a, I> where I: IntoIterator<Item = f64> {}

enum QuantilesInner<'a, I>
where
    I: IntoIterator<Item = f64>,
{
    // Repeat the iterator until we have exhausted quantiles.
    Repeat(I::IntoIter, f32),
    // Start processing the next centroid in the list.
    NextCentroid(I::IntoIter, QuantilesVariables<'a>),
    // Yield quantiles for the current centroid.
    IterCentroid(I::IntoIter, QuantilesVariables<'a>),
    // Update state variables before moving to the next centroid.
    ExitCentroid(I::IntoIter, QuantilesVariables<'a>),
    Exhausted,
}

impl<'a, I> QuantilesInner<'a, I>
where
    I: IntoIterator<Item = f64>,
{
    fn into_next(self) -> (Self, Option<f32>) {
        match self {
            Self::Repeat(mut levels, quantile) => {
                if levels.next().is_none() {
                    (Self::Exhausted, None)
                } else {
                    (Self::Repeat(levels, quantile), Some(quantile))
                }
            }
            Self::NextCentroid(levels, mut vars) => {
                vars.c = match vars.centroids.next() {
                    None => {
                        return (Self::Repeat(levels, vars.c.mean), Some(vars.c.mean));
                    }
                    Some(c) => *c,
                };
                vars.current_x = vars.sum as f64 + vars.c.count as f64 * 0.5;
                if vars.current_x < vars.x {
                    return Self::ExitCentroid(levels, vars).into_next();
                }
                vars.left = vars.prev_x;
                if vars.prev_c.count == 1 {
                    vars.left += 0.5;
                }
                vars.right = vars.current_x;
                if vars.c.count == 1 {
                    vars.right -= 0.5;
                }
                Self::IterCentroid(levels, vars).into_next()
            }
            Self::IterCentroid(mut levels, mut vars) => {
                if vars.current_x < vars.x {
                    return Self::ExitCentroid(levels, vars).into_next();
                }
                let quantile = {
            ...
            Self::ExitCentroid(levels, mut vars) => {
                vars.sum += vars.c.count;
                vars.prev_c = vars.c;
                vars.prev_x = vars.current_x;
                Self::NextCentroid(levels, vars).into_next()
            }
            Self::Exhausted => (Self::Exhausted, None),
        }
    }
}

struct QuantilesVariables<'a> {
    centroids: std::slice::Iter<'a, Centroid>,
    count: usize,
    x: f64,
    prev_x: f64,
    current_x: f64,
